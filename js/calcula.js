...
function igualar() {
 ...
 function binario(){  // Decimal a binario
         res = sol.toString(2); // A la base 2
         pantalla.innerHTML=res;
         op="no"; //ya no hay operaciones pendientes
         xi=1; //se puede reiniciar la pantalla.
         res=0;
         }
 function octal(){  // Decimal a octal
         oc = sol.toString(8); // A la base 8
         pantalla.innerHTML=oc;
         op="no"; //ya no hay operaciones pendientes
         xi=1; //se puede reiniciar la pantalla.
         oc=0;
          }
 function hexadecimal(){ // Decimal a hexadecimal
          hex = sol.toString(16); // A la base 16
          pantalla.innerHTML=hex;
          op="no"; //ya no hay operaciones pendientes
          xi=1; //se puede reiniciar la pantalla.
          hex=0;
          }
...
function cuad() {
         x=x*x; //multiplicar al cuadrado el número
         pantalla.innerHTML=x; //mostrar en pantalla
         igualar() //resolver y mostrar operaciones pendientes
         xi=1 //reiniciar la pantalla
         }
...
function asin() {
         x=Math.asin(x)
         pantalla.innerHTML=x; //mostrar en pantalla
         //resolver y mostrar operaciones pendientes
         xi=1 //reiniciar la pantalla
         }
function acos() {
         x=Math.acos(x);
